// referanced: Packages/com.unity.render-pipelines.universal/ShaderLibrary/GlobalIllumination.hlsl

#ifndef TOON_GLOBAL_ILLUMINATION_INCLUDED
#define TOON_GLOBAL_ILLUMINATION_INCLUDED
...
half3 SampleSHVertex_Toon(half3 normalWS)
{
...
half3 SampleSHPixel_Toon(half3 L2Term, half3 normalWS)
{
#if defined(EVALUATE_SH_VERTEX)
    return L2Term;
#elif defined(EVALUATE_SH_MIXED)
...
#if (defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2))
half3 SampleProbeVolumeVertex_Toon(in float3 absolutePositionWS, in float3 normalWS, in float3 viewDir, out float4 probeOcclusion)
{
    probeOcclusion = 1.0;
...
half3 SampleProbeVolumePixel_Toon(in half3 vertexValue, in float3 absolutePositionWS, in float3 normalWS, in float3 viewDir, in float2 positionSS, in float4 vertexProbeOcclusion, out float4 probeOcclusion)
{
    probeOcclusion = 1.0;
...
half3 SampleProbeVolumePixel_Toon(in half3 vertexValue, in float3 absolutePositionWS, in float3 normalWS, in float3 viewDir, in float2 positionSS)
{
...
half3 SampleProbeSHVertex_Toon(in float3 absolutePositionWS, in float3 normalWS, in float3 viewDir, out float4 probeOcclusion)
{
    probeOcclusion = 1.0;
...
half3 SampleProbeSHVertex_Toon(in float3 absolutePositionWS, in float3 normalWS, in float3 viewDir)
{
    float4 unusedProbeOcclusion = 0;
...
half3 SampleLightmap_Toon(float2 staticLightmapUV, float2 dynamicLightmapUV, half3 normalWS)
{
...
half3 SampleLightmap_Toon(float2 staticLightmapUV, half3 normalWS)
{
...
half3 BoxProjectedCubemapDirection_Toon(half3 reflectionWS, float3 positionWS, float4 cubemapPositionWS, float4 boxMin, float4 boxMax)
{
...
float CalculateProbeWeight_Toon(float3 positionWS, float4 probeBoxMin, float4 probeBoxMax)
{
...
half CalculateProbeVolumeSqrMagnitude_Toon(float4 probeBoxMin, float4 probeBoxMax)
{
    half3 maxToMin = half3(probeBoxMax.xyz - probeBoxMin.xyz);
    return dot(maxToMin, maxToMin);
}

half3 CalculateIrradianceFromReflectionProbes_Toon(half3 reflectVector, float3 positionWS, half perceptualRoughness, float2 normalizedScreenSpaceUV)
{
...
half3 GlossyEnvironmentReflection_Toon(half3 reflectVector, float3 positionWS, half perceptualRoughness, half occlusion, float2 normalizedScreenSpaceUV)
{
...
#if !USE_FORWARD_PLUS
half3 GlossyEnvironmentReflection_Toon(half3 reflectVector, float3 positionWS, half perceptualRoughness, half occlusion)
{
    return GlossyEnvironmentReflection_Toon(reflectVector, positionWS, perceptualRoughness, occlusion, float2(0.0f, 0.0f));
}
#endif

half3 GlossyEnvironmentReflection_Toon(half3 reflectVector, half perceptualRoughness, half occlusion)
{
...
{
    return GlobalIllumination_Toon(brdfData, brdfDataClearCoat, clearCoatMask, bakedGI, occlusion, positionWS, normalWS, viewDirectionWS, float2(0.0f, 0.0f));
}
#endif
...
void MixRealtimeAndBakedGI_Toon(inout Light light, half3 normalWS, inout half3 bakedGI)
{
...
{
    MixRealtimeAndBakedGI_Toon(light, normalWS, bakedGI);
}

void MixRealtimeAndBakedGI_Toon(inout Light light, half3 normalWS, inout half3 bakedGI, AmbientOcclusionFactor aoFactor)
{
